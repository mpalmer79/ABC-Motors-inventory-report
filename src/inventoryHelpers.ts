...
export const getModelColor = (name: string, index: number): string => {
    ...
    return CHART_COLORS[index % CHART_COLORS.length] ?? ABC_BLUE;
};
...
export function exportToCsv(filename: string, rows: InventoryRow[]): void {
    ...
    const headers = [
          "Stock Number", "Year", "Make", "Model", "Exterior Color",
          "Trim", "Model Number", "Cylinders", "VIN", "Age", "MSRP",
        ];
    ...
    rows.forEach((r) => {
          lines.push(
                  [r["Stock Number"], r.Year, r.Make, r.Model, r["Exterior Color"],
                          r.Trim, r["Model Number"], r.Cylinders, r.VIN, r.Age, r.MSRP]
                    .map(escape).join(",")
                );
    });
